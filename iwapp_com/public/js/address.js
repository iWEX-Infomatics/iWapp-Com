frappe.ui.form.on("Address", {
    pincode: function (frm) {
        ...
        if (frm.doc.country == "India" && frm.doc.pincode) {
            frappe.db.exists('Pincode', frm.doc.pincode)
                .then(exists => {
                    if (exists) {
                        frappe.db.get_doc('Pincode', frm.doc.pincode)
                            .then(doc => {
                            ...
                            callback: function (r) {
                                if (r.message) {
                                    ...
                                    $.each(r.message, function (i, pin) {
                                        frm.set_value({ "state": pin.State, "custom_taluk": pin.Block, "custom_county": pin.District })
                                    ...
                                    frm.set_value({ "state": "", "custom_taluk": "", "custom_county": "", "custom_post_office": "" })
                                }
                            }
                        })
                    }
